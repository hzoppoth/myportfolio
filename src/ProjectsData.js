...
const projects = [
    ...
    },
    {
        ...
        features: ['Automated Data Entry: Streamlines the cask filling process by automatically fetching and storing relevant data from user inputs into the database.','Traceability and Reporting: Once submitted, a deatailed traceback row is created in the database, allowing for proper batch tracking and historical data analysis.','Inuitive Design: The layout is simple and dropdowns are provided for user input. Volume numbers are calculated in real time to give users confirmation on the screen before submitting. The user is also given the traceback ID once the submit action has completed and the entry was logged without errors.'],
